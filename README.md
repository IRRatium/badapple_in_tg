...
> ⚠️ **Лимиты Bot API (RetryAfter):** из-за ограничений Telegram на создание стикерпаков вероятно потребуется несколько ботов (или несколько аккаунтов Telegram). Один бот вытягивает примерно **~9 фрагментов по 3 секунды**, после чего начинают сыпаться `RetryAfter`. Заведи дополнительных ботов через [@BotFather](https://t.me/BotFather) и запускай нарезку под разными аккаунтами.
...
Для `bot.py` — все 5 ботов должны быть администраторами чата с правами **«Публикация»**, **«Редактирование сообщений»** и **«Удаление сообщений»** (бот удаляет старое сообщение при каждой смене партии ботов).
